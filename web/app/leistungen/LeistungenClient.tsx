...
// Helper to normalize any ID to a clean 'service-{slug}' format
const getServiceDomId = (id: string) => {
  const clean = id.startsWith("service-") ? id.slice(8) : id;
  return `service-${clean}`;
};

// Helper to normalize string for fuzzy matching (remove dashes, spaces, hashes, umlauts)
...
// Robust matcher that finds the target service by ID, title, or keywords
const findMatchingService = (services: ServiceItemData[], key: string): ServiceItemData | null => {
  if (!key) return null;
  ...
  let found = services.find(
    (s) => s._id === key || s._id === `service-${key}` || getServiceDomId(s._id) === getServiceDomId(key)
  );
  if (found) return found;

  // 2. Normalized ID match
  found = services.find((s) => normalizeKey(s._id) === cleanKey);
  if (found) return found;

  // 3. Title match
  found = services.find((s) => {
    const normTitle = normalizeKey(s.title);
    return normTitle.includes(cleanKey) || cleanKey.includes(normTitle);
  });
  if (found) return found;

  // 4. Keyword fuzzy matching for common trade terms
  if (cleanKey.includes("tuer") || cleanKey.includes("haustuer")) {
    found = services.find((s) => normalizeKey(s.title).includes("tuer"));
    if (found) return found;
  }
  if (cleanKey.includes("wintergaert") || cleanKey.includes("wintergarten") || cleanKey.includes("glasbau")) {
    found = services.find((s) => normalizeKey(s.title).includes("wintergaert") || normalizeKey(s.title).includes("wintergarten"));
    if (found) return found;
  }
  if (cleanKey.includes("tor") || cleanKey.includes("garage")) {
    found = services.find((s) => normalizeKey(s.title).includes("tor"));
    if (found) return found;
  }
  if (cleanKey.includes("fenster") && !cleanKey.includes("alu") && !cleanKey.includes("kunststoff")) {
    found = services.find((s) => normalizeKey(s.title).includes("holzfenster"));
    if (found) return found;
  }
  if (cleanKey.includes("alu") || cleanKey.includes("mira")) {
    found = services.find((s) => normalizeKey(s.title).includes("alu") || normalizeKey(s.title).includes("mira"));
    if (found) return found;
  }
  if (cleanKey.includes("kunststoff") || cleanKey.includes("veka") || cleanKey.includes("gealan")) {
    found = services.find((s) => normalizeKey(s.title).includes("kunststoff") || normalizeKey(s.title).includes("veka"));
    if (found) return found;
  }
  if (cleanKey.includes("rollladen") || cleanKey.includes("beschattung")) {
    found = services.find((s) => normalizeKey(s.title).includes("rollladen"));
    if (found) return found;
  }
  if (cleanKey.includes("innentuer")) {
    found = services.find((s) => normalizeKey(s.title).includes("innentuer"));
    if (found) return found;
  }
...
// Safe DOM element retriever trying all ID variations
const getElementForService = (service: ServiceItemData): HTMLElement | null => {
  return (
    document.getElementById(getServiceDomId(service._id)) ||
    document.getElementById(`service-${service._id}`) ||
    document.getElementById(service._id)
  );
};

// Scroll to element with exact header offset
const scrollToServiceElement = (el: HTMLElement) => {
...
export default function LeistungenClient({
  ...
  // Smoothly scroll to the target service item when toggled by user on the page
  useEffect(() => {
    if (!scrollTargetRef.current) return;
    const targetId = scrollTargetRef.current;
    scrollTargetRef.current = null;

    const matchedService = services.find((s) => s._id === targetId);
    if (!matchedService) return;

    // Small delay ensures previous item unmounts and DOM layout settles before measuring
    const timer = setTimeout(() => {
      const el = getElementForService(matchedService);
      if (el) {
        scrollToServiceElement(el);
      }
    }, 80);

    return () => clearTimeout(timer);
  }, [expandedIds, services]);

  // Handle URL hash or search parameter on load & when navigating between pages (e.g. from Homepage)
  useEffect(() => {
    ...
    const handleHashOrSearch = () => {
      ...
      if (hash) {
        targetKey = hash.replace(/^#/, "");
      } else if (search) {
        const params = new URLSearchParams(search);
        targetKey = params.get("service") || params.get("gewerk") || "";
      }

      if (!targetKey) {
        if (defaultExpandedFirst && services.length > 0 && expandedIds.length === 0) {
          setExpandedIds([services[0]._id]);
        }
        return;
      }

      const matched = findMatchingService(services, targetKey);
      if (matched) {
        // Expand the target service immediately
        setExpandedIds([matched._id]);

        // Staggered scroll attempts to handle DOM mounting, font loading & layout reflow
        const executeScroll = () => {
          const el = getElementForService(matched);
          if (el) {
            scrollToServiceElement(el);
          }
        };

        // 1st frame: immediate after React commits DOM
        const t1 = setTimeout(executeScroll, 60);
        // 2nd frame: after browser layout reflow
        const t2 = setTimeout(executeScroll, 200);
        // 3rd frame: ensure stable position after any fonts or layout shifts
    ...
    const cleanup = handleHashOrSearch();

    // Listen to hash changes in case user clicks anchor links while on the page
      ...
      <div className="w-full">
        ...
        <div className="divide-y divide-[#E8E8E6]">
          {services.map((item, idx) => {
            ...
            // Clean DOM element ID without double 'service-service-' prefixes
            const domId = getServiceDomId(item._id);
